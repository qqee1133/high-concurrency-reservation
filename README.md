...
-   [x] **4단계: Redis 메시지 큐 (트래픽 제어)**
    -   **가설:** 1만 명의 요청을 직접 처리하지 않고, `Redis`의 **List**에 큐잉하여 시스템을 안정화한다.
    -   [x] 4-1. API (요청 발행): 예약 요청을 즉시 `Redis List`에 저장하고 접수 완료 응답.
    -   [x] 4-2. Consumer (요청 처리): 별도 스레드가 큐에서 요청을 꺼내어 2/3단계의 안전한 로직으로 **순차적** 재고 차감 처리.
...
#### 1. API 응답 속도 (Producer)
![4단계 API 접수 속도](images/step4_api_result.png)

-   **총 수행 시간:** 223 ms (1000건)
-   **분석:** 사용자의 대기 시간이 **0.22초**로 획기적으로 단축됨. DB를 전혀 거치지 않고 Redis에 넣기만 하므로, 트래픽이 폭주해도 API 서버는 죽지 않고 안정적으로 응답함.

#### 2. 최종 처리 결과 (Consumer)
![4단계 최종 DB 처리 결과](images/step4_db_result.png)
...
### `POST /reservations`
-   **설명:** 사용자 예약 요청 (4단계 메시지 큐 기준)
-   **동작:** 요청을 Redis 대기열에 적재하고 즉시 응답을 반환합니다.
-   **요청 본문:** `{"productId": 1, "userId": 123}`
-   **성공 응답 (200 OK):** (Empty Body)
-   **참고:** 요청은 즉시 접수되며, 실제 성공 여부는 Consumer가 처리한 뒤 결정됩니다.

### `GET /products/{id}/stock`
-   **설명:** 상품의 현재 재고 조회
-   **성공 응답 (200 OK):** `{"stock": 100}`